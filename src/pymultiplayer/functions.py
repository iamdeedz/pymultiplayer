...
async def get_servers(ip, port, ws_or_wss: str ="ws"):
    async with websockets.connect(f"{ws_or_wss}://{ip}:{port}") as websocket:
...
async def create_server(ip, port, parameters: dict, ws_or_wss: str ="ws"):
    async with websockets.connect(f"{ws_or_wss}://{ip}:{port}") as websocket:
...
async def get_player_count_of_server(ip, port, ws_or_wss: str ="ws"):
    async with websockets.connect(f"{ws_or_wss}://{ip}:{port}") as websocket:
